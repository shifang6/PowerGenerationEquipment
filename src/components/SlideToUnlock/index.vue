...
export default {
  ...
  methods: {
      ...
      this.register().then(
        (respData) => {
          ...
          if (respData && respData.code === 0) {
            this.sliceImg = respData.data.sliceImg;
            this.bgImg = respData.data.bgImg;
            this.token = respData.data.token;
            this.sliceY = respData.data.y;
          } else {
            this.$message({
              type: "error",
              showClose: true,
    ...
    checkCaptcha() {
      ...
      this.login(this.token, this.sliceX).then(
        (respData) => {
          if (respData.code === 0) {
            this.successStyle();
            this.$message({
